import Head from 'next/head'
import Image from 'next/image'
import Link from 'next/link'
import Layout from '../components/Layout'
import utilStyles from '../styles/utils.module.css'

export default function Home() {
  return (
    <Layout home>
      <Head>
        <title>Personal Portfolio</title>
      </Head>
        <div className="container mx-auto  flex flex-col items-center justify-center w-full mt-16 flex-1 px-20 text-center opacity-75" >

          <h1 className="text-6xl font-bold">
            Welcome to my Page
            
          </h1>

          <p className="mt-3">
            Built using <code> Next.js</code> and <code>Tailwind CSS</code>
          </p>

          <div className="flex items-center justify-around max-w-4xl mt-6 sm:w-full">
            <div className="flex  mx-auto mt-6 text-left border space-x-4 rounded-xl hover:text-blue-600 focus:text-blue-600">
              <img alt="mountain" className="w-45 rounded-md border-2 border-gray-300" src="https://picsum.photos/seed/picsum/200" />
              <Link href="./about">
                <a className="flex flex-col justify-center p-4">
                  <h3 className="text-2xl font-bold">My Portfolio &rarr;</h3>
                  <p className="mt-4 text-xl">
                    Find out more about me and my work experience. 
                  </p>
                </a>
              </Link>
            </div>

            <div className="flex  mt-6 text-left border space-x-4 rounded-xl hover:text-blue-600 focus:text-blue-600">
            <img alt="mountain" className="w-45 rounded-md border-2 border-gray-300" src="https://picsum.photos/seed/picsum/200" />
              <Link href="./blog">
                <a className="flex flex-col justify-center p-4">
                  <h3 className="text-2xl font-bold">My Blog &rarr;</h3>
                  <p className="mt-4 text-xl">
                    Check out my latest posts and reviews on my blog!
                  </p>
                </a>
              </Link>
            </div>
          </div>
        </div>
      
    </Layout>

  )
}
