...
import { changeStatus } from '../lib/auth'
...
export default function Nav({loggedOut} : {loggedOut?: boolean}) {
      ...
      <div className="w-full flex items-center justify-between mt-0 px-6 py-2">
       ...
       <div className="order-2 md:order-3 flex flex-wrap items-center justify-end mr-0 md:mr-4" id="nav-content">
          <div className="auth flex items-center w-full md:w-full">
             <button className="bg-white text-indigo-600  p-2 rounded border mr-4 hover:bg-gray-100 hover:text-indigo-700" onClick={loggedOut => !loggedOut}>
               { loggedOut ? (
                  <Link href="./login">
                     <a>Sign In</a>
                  </Link>) : (
